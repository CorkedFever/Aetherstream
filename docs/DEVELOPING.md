...
| [South Park Studios](https://www.southparkstudios.com/) | The South Park app: one list of what still plays free, as stills | Paramount's own site for the show, free with ads, though most episodes have moved to Paramount+ and only a few per season still play. The season list is read from a season page and a season's episodes from the site's paging API with title, number, still and blurb. Neither says what plays (the page marks only its first ten cards), so the player service is asked by episode id, `topaz.paramount.tech/topaz/api/mgid:arc:episode:shared.southpark.us.en:{id}/mica.json?clientPlatform=desktop`, which answers with a `stitchedstream` or an error slate; eight at a time the whole catalogue takes a few seconds, and it is kept in `southpark.json` for twelve hours. An episode plays from that service's master playlist, reached through its page (`?json=true` → `videoDetail.videoServiceUrl`), not through yt-dlp: that hands the sound over as a second playlist, and libvlc froze on it. The sound is a television master, lifted six decibels. |
| [Adult Swim](https://www.adultswim.com/streams) | The Adult Swim app: the marathon streams, with what each is showing | The streams from `/api/streams/v1/streams` (id, title, blurb, posters, the marathon id, the platforms it is for), what each is showing from `/api/schedule/marathons/{marathonId}` (name, series, start, duration), and a stream's playlist from `/api/shows/v1/videos/{streamId}?fields=stream`: a plain AES-128 HLS master at up to 480p, no token or login. yt-dlp's extractor for the site is broken, so this is the plugin's own. The sound is a television master, lifted eight decibels. |
...
- **`:input-slave=` is not a mux.** libvlc runs the second input on its own clock and only nudges
  it toward the first. That holds for YouTube's plain segments and fell apart on South Park
  Studios' AES-encrypted, ad-stitched pair: `playback way too early … playing silence`, `buffer
  deadlock prevented`, and the picture stuck on its first frame at 0:00. When a site publishes a
  master playlist with the sound as an `EXT-X-MEDIA` rendition, hand libvlc the master and let its
  one demuxer keep both in step.
- **`:audio-filter=` as a media option does nothing.** libvlc 3's audio output reads its filter
  chain from the instance, not the item, so a compressor or gain passed with the media is ignored
  without a word; the harness measured the same level with and without. The one per-player gain
  there is goes through the equalizer's preamp, whose flat point is 12 dB (VLC's own default), so
  `VlcStreamSource.SetGain` sets the preamp to 12 plus the decibels and tops out at +8. Television
  masters sit well under the rest (South Park about -24 dBFS RMS, Adult Swim's marathons about
  -28, a YouTube video about -18), so their resolvers ask for a lift through
  `ResolvedStream.GainDb` and the session applies it after `Play`.
