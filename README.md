...
| Kitchen | A cooking show. Every episode is a real Culinarian recipe: a title card with the dish, a card per ingredient with its icon, amount and a line of patter, a method, and the plate with the item's own description; about ninety seconds an episode, on a schedule everyone sees alike | The Recipe and Item tables and the game's icon textures through Lumina; crystals are left off the list; the patter is written |
