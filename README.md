...
| Stories | The bard's hour: the tales of the Twelve, two for each god, six pages apiece, told the old way from a chair by the fire. The teller is a silhouette against the firelight, book on the knee and lantern raised; each page is read from the chair and then shown as a plate in a shadow play, the god in the sky in colour, from the game's own render, over the country the tale is set in in cut paper and the tale's mortals as cutouts below; the Traders stand side by side. A shuffled round gives every god a turn before any gets a second | Written, in the register of the almanacs and the temple tellings, on what the game says of each god: domain, element, symbol, city, moon and the few kinships everyone knows. Nothing is quoted from the game's text |
