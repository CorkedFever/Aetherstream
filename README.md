...
| [Final Fantasy Wiki](https://finalfantasy.fandom.com/) renders | The story hour's Scions, as sprites | Square Enix's official character renders, pixelated to fifty-six rows by `tools/make_scions.py` from the links in `tools/scion_renders.json` and bundled as `data/scions.json.gz`. Fan use, inside a plugin for their own game. |
...
| Wildlife | A moogle in a bush hat and a khaki vest, Stevie Mogwyn, and a creature from the hunting log every episode: the approach through its country, a close-up with a fact card, the moment he gets far too close and has to run, the sign-off; backdrops by the creature's region, the sky by your hour, a next-week card | The MonsterNoteTarget table for creatures, zones and portraits, the TerritoryType table for regions; the narration is written |
| Stories | The bard's hour as a shadow play: the teller a silhouette in a high-backed chair against the firelight, book on the knee and lantern raised, and a tale a time in six pages, each read from the chair then shown as a plate on a backlit paper screen, every figure a black cutout, or in colour when the tale is about a Scion, drawn from their render: a hero of one of the peoples with a made-up name in their fashion, or every other tale one of the Scions with their own trade, a road between places that exist, a beast from the bestiary, a treasure from the item table, a god from the Twelve; he ends every tale the same way and you don't have to take his word for it | The outdoor zones and their regions, the hunting log, and the item table; the pages are written and seeded by the tale |
