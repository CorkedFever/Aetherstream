...
| Hunt photos (your own) | The wildlife show's WANTED posters and fact cards for marks | Optional. Drop screenshots of marks into `tools/hunt_photos/` named after the mark, run `tools/make_hunt_photos.py`, and they ship as `data/hunts.json.gz`. Gamer Escape's monster pages carry one per mark; its image host only talks to browsers, so they are saved by hand. |
