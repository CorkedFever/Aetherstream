...
| Stories | The bard's hour as a shadow play: the teller a silhouette in a high-backed chair against the firelight, book on the knee and lantern raised, and a tale a time in six pages, each read from the chair then shown as a plate on a backlit paper screen, every figure a black cutout: a hero of one of the peoples, a road between places that exist, a beast from the bestiary, a treasure from the item table, a god from the Twelve; he ends every tale the same way and you don't have to take his word for it | The outdoor zones and their regions, the hunting log, and the item table; the pages are written and seeded by the tale |
